...
com_tamina_cow4_IADemoApp.prototype = {
	...
	,processTurn: function(data) {
		...
		if(wait) console.log("wait"); else {
			...
			var path = com_tamina_cow4_utils_GameUtils.getPath(currentCell,sheepCell,gameData);
			var _g1 = 0;
			var _g = myIa.pm;
			while(_g1 < _g) {
				var i = _g1++;
...
var com_tamina_cow4_model_IAInfo = function(id,name,avatar,pm) {
	this.pm = 1;
	this.id = id;
	this.name = name;
	this.avatar = avatar;
...
com_tamina_cow4_socket_Proxy.prototype = {
	...
	,socketServer_endHandler: function() {
		try {
			...
			this._data = "";
			if(message.type != null) this.messageSignal.dispatch(message); else this.sendError(new com_tamina_cow4_socket_message_Error(2,"message inconnu"));
		} catch( e ) {
